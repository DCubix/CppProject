...
void Knob::onDraw(NVGcontext* ctx, float deltaTime) {
	...
	nvgBeginPath(ctx);
	nvgArc(ctx, b.width / 2, yPos, radius, angleFrom + gap * 2, angleFrom, NVG_CW);
	nvgArc(ctx, b.width / 2, yPos, radius - thickness, angleFrom, angleFrom + gap * 2, NVG_CCW);

	nvgFillColor(ctx, nvgRGBAf(0.0f, 0.0f, 0.0f, 0.6f));
	nvgFill(ctx);

	nvgBeginPath(ctx);
	nvgArc(ctx, b.width / 2, yPos, radius - 1, angleFrom + gap * 2, angleFrom + gap * 2 + angleSpan * nvalue, NVG_CW);
	nvgArc(ctx, b.width / 2, yPos, radius + 1 - thickness, angleFrom + gap * 2 + angleSpan * nvalue, angleFrom + gap * 2, NVG_CCW);
	...
	nvgText(ctx, b.width / 2, yPos + b.height / 2.0f, valueText.c_str(), nullptr);
}
